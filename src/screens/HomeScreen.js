...
const Home = ({ navigation }) => {
  ...
  const startScan = async () => {
    try {
      ...
      // Get already paired/bonded devices first (BT-01 usually needs pairing)
      const paired = await RNBluetoothClassic.getBondedDevices();
      if (paired && paired.length > 0) {
        setDevices(paired);
      }

      // Also discover new unpaired devices
      ...
      // Discovery runs for ~30s automatically, but we cancel after 30s
      setTimeout(async () => {
        try {
      ...
      // Merge discovered devices with paired list
      if (discovering && discovering.length > 0) {
        setDevices((prev) => {
          const merged = [...prev];
          discovering.forEach((d) => {
            if (!merged.find((p) => p.address === d.address)) {
              merged.push(d);
            }
          });
          return merged;
  ...
  const DeviceCard = ({ item }) => (
    ...
    >
      ...
      <View style={styles.deviceInfo}>
        {/* Device name — BT-01 usually advertises as "HC-05", "BT01", etc. */}
        <Text style={styles.deviceName}>
          {item.name || item.address || "Unknown Device"}
        </Text>
        ...
        {isPaired(item) && (
          <View style={styles.pairedBadge}>
            <Icon name="link-variant" size={10} color="#16a34a" />
      ...
      <View style={styles.deviceRight}>
        <View
          style={[
            styles.signalBadge,
            { borderColor: getSignalColor(item.rssi) + "40" },
          ]}
        >
          <Icon
          ...
          />
          <Text
            style={[styles.signalLabel, { color: getSignalColor(item.rssi) }]}
          >
  ...
  const EmptyState = () => (
    <View style={styles.emptyWrap}>
      ...
      <Text style={styles.emptySubtitle}>
        Make sure your BT-01 module is powered on and in pairing mode (LED
        blinking fast)
      </Text>
    ...
    <View style={styles.container}>
      ...
      <View style={styles.statusBar}>
        ...
        <Text style={styles.statusText}>
          {scanning
            ? `Scanning for devices… ${countdown}s`
      ...
      <Text style={styles.footer}>
        Tap a device to connect · Classic Bluetooth (SPP)
      </Text>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            ...
            <Text style={styles.modalSubtitle}>
              You'll be disconnected from all Bluetooth devices and returned to
              the login screen.
            </Text>
...
const styles = StyleSheet.create({
  ...
  decorCircleSmall: {
    ...
    backgroundColor: "#eff6ff", opacity: 0.9,
  },
  header: {
  ...
  logoShadow: {
    borderRadius: 14, shadowColor: "#2563eb",
  ...
  iconBtn: {
    ...
    borderColor: "#bfdbfe", justifyContent: "center", alignItems: "center",
  },
  ...
  statusBar: {
    ...
    shadowOpacity: 0.06, shadowRadius: 8, elevation: 3, gap: 8,
  },
  statusDot: { width: 8, height: 8, borderRadius: 4, backgroundColor: "#cbd5e1" },
  statusDotActive: { backgroundColor: "#22c55e" },
  statusText: { flex: 1, fontSize: 13, color: "#475569", fontWeight: "500" },
  deviceCountBadge: {
  ...
  deviceCountText: { fontSize: 12, fontWeight: "700", color: "#2563eb" },
  sectionRow: {
    flexDirection: "row", alignItems: "center",
  ...
  scanningDot: { width: 6, height: 6, borderRadius: 3, backgroundColor: "#22c55e" },
  scanningPillText: { fontSize: 11, fontWeight: "700", color: "#16a34a" },
  listContent: { paddingBottom: 10, flexGrow: 1 },
  deviceCard: {
    flexDirection: "row", alignItems: "center",
    ...
    shadowOpacity: 0.07, shadowRadius: 10, elevation: 4, gap: 12,
  },
  deviceIconBg: {
  ...
  deviceInfo: { flex: 1 },
  deviceName: { fontSize: 15, fontWeight: "700", color: "#0f172a", marginBottom: 3 },
  deviceId: { fontSize: 11, color: "#94a3b8", fontWeight: "500" },
  pairedBadge: {
    flexDirection: "row", alignItems: "center", gap: 3,
    marginTop: 4, backgroundColor: "#f0fdf4",
    borderRadius: 6, paddingHorizontal: 6, paddingVertical: 2,
    alignSelf: "flex-start", borderWidth: 1, borderColor: "#bbf7d0",
  },
  pairedText: { fontSize: 10, fontWeight: "700", color: "#16a34a" },
  deviceRight: { alignItems: "flex-end", gap: 6 },
  signalBadge: {
    flexDirection: "row", alignItems: "center", gap: 4,
    borderRadius: 8, paddingHorizontal: 7, paddingVertical: 3,
    borderWidth: 1, backgroundColor: "#f8fafc",
  },
  signalLabel: { fontSize: 10, fontWeight: "700" },
  connectBtn: {
  ...
  emptyWrap: {
    ...
    paddingTop: 50, paddingHorizontal: 30,
  },
  emptyIconCircle: {
  ...
  footer: {
    ...
    fontWeight: "500", marginTop: 14, letterSpacing: 0.2,
  },
  modalOverlay: {
  ...
  modalIconCircle: {
    ...
    backgroundColor: "#fff1f2", borderWidth: 1.5,
    borderColor: "#fecaca", justifyContent: "center",
    alignItems: "center", marginBottom: 16,
  },
  ...
  logoutBtnWrap: {
    flex: 1, borderRadius: 14, shadowColor: "#ef4444",
    shadowOffset: { width: 0, height: 6 },
